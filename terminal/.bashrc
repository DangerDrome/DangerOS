...
# Source aliases: grab aliases from the '.bash_aliases' file
...
# eval "$(oh-my-posh --init --shell bash --config ~/.poshthemes/tokyo.omp.json)"
...
install_bashrc_support ()
{
	...
	if [ $dtype == "redhat" ]; then
		sudo yum install multitail tree exa
	elif [ $dtype == "slackware" ]; then
...
function __setprompt
{
	...
	# PS1+="[Size:\[${GREEN}\]$(/bin/ls -lah | /bin/grep -m 1 total | /bin/sed 's/total //')\[${DARKGRAY}\] "

	# Number of files
	# PS1+="Files:\[${GREEN}\]\$(/bin/ls -A -1 | /usr/bin/wc -l)\[${DARKGRAY}\]]"
